...
**结论**：若$f(x,y)$可微，且$f(x,y)$是n次齐次函数，则
$$
...
**定义**：
 - **极大值**：$f(x_0,y_0)\geq f(x,y)$（局部最高点）
 - **极小值**：$f(x_0,y_0)\leq f(x,y)$（局部最低点）

**极值的必要条件**：$f_x'(x_0,y_0)=0\,\,,f_y'(x_0,y_0)=0$（可导）
 - 极值点$\nrightarrow$驻点，驻点$\nrightarrow$极值点
 - 若$f(x,y)$可导，则==极值点一定是驻点==

**极值的充分条件**：设$f_x'(x_0,y_0)=0\,\,,f_y'(x_0,y_0)=0$（A是对x的二阶偏导，C是对y的二阶偏导，B是对x，y的混合偏导）
 1. 当$AC-B^2>0$时，**有极值**
     - A>0 **极小值**
     - A<0 **极大值**
 2. 当$AC-B^2<0$时，**无极值**
 3. 当$AC-B^2=0$时，**不一定**（一般用定义法判定）

## 条件极值与拉格朗日乘数法

### 函数$f(x,y)$在条件$φ(x,y)=0$条件下的极值

几何意义：找曲线对应到曲面上的线上的局部最低（高）点

**方法**：拉格朗日乘数法
**求解**：令$F(x,y,\lambda)=f(x,y)+\lambda\varphi(x,y)$，求解方程组：
$$
\left\{
\begin{matrix}
...
### 函数$f(x,y,z)$在条件$φ(x,y,z)=0,ψ(x,y,z)=0$条件下的极值

方法：拉格朗日乘数法
**求解**：令$F(x,y,z,\lambda,\mu)=f(x,y,z)+\lambda\varphi(x,y,z)+\mu\psi(x,y,z)$，解方程组：
$$
\left\{
\begin{matrix}
F_x=f_x'(x,y,z)+\lambda\varphi_x'(x,y,z)+\mu\psi_x'(x,y,z)=0\\
F_y=f_y'(x,y,z)+\lambda\varphi_y'(x,y,z)+\mu\psi_y'(x,y,z)=0\\
F_z=f_z'(x,y,z)+\lambda\varphi_z'(x,y,z)+\mu\psi_z'(x,y,z)=0\\
