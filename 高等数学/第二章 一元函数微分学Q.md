...
**方法**：
 1. 单调性
 2. **罗尔定理推论**：若在区间$I$上$f^{(n)}(x)\neq0$，则方程$f(x)=0$在$I$上**最多**n个实根（最多，并不是一定）
**工具**：可以尝试分离参数
...
## <span style="color : red;">题型5 微分中值定理证明题</span>

### 类型1 证明一点ξ 使得 F(ξ,f(ξ),f'(ξ))=0

<span style="color : red;font-weight:bold;">方法：构造辅助函数用罗尔定理</span >

**分析法**（还原法）：根据预证明的结论$F[\xi,f(\xi),f'(xi)]=0$的分析，确定$g(x)$，使得$g'(x)=F[x,f(x),f'(x)]$

**微分方程法**：预证明$F[\xi,f(\xi),f'(xi)]=0$
 1. 求微分方程$F(x,y,y')=0$的通解$H(x,y)=C$
 2. 构造辅助函数：$g(x)=H(x,f(x))$

**注**：
![[常见辅助函数构造]]

### 类型2 证明两点ξ,η，使得F(ξ,η,f(ξ),f(η),f'(ξ),f'(η))=0

**方法**：
 1. 不要求$\xi\neq\eta$：
     - 同一区间$[a,b]$上用两次中值定理（拉格朗日中值定理、柯西中值定理）
 2. 要求$\xi\neq\eta$：
     - 将区间$[a,b]$分为**两个子区间**，在两个子区间上分别使用拉格朗日中值定理


### 类型3 证明存在一个中值点ξ，使得$F(ξ,f^{(n)}(ξ))≥0$

**方法**：使用带==拉格朗日余项的泰勒公式==，其中$x_0$点选择题目中提供函数值和导数值信息多的点
 - *多项式拟合：找一个多项式g(x)满足f(x)，令F(x)=f(x)-g(x)*
